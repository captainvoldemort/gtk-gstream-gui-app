...
static void start_pipeline(AppData *app_data) {
    // Set the pipeline to the PAUSED state initially
    gst_element_set_state(app_data->pipeline, GST_STATE_PAUSED);

    // Start the GTK main loop
    gtk_main();

    // Set the pipeline to the PLAYING state after the GTK main loop has started
...
int main(int argc, char *argv[]) {
    ...
    gst_element_get_state(app_data.pipeline, NULL, NULL, GST_CLOCK_TIME_NONE);
