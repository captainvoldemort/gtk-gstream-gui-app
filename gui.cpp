...
static void setup_home_screen(AppData *app_data) {
    ...
    gtk_widget_set_name(app_data->speedLabel, "speedLabel");
...
int main(int argc, char *argv[]) {
    ...
    // Apply CSS to the speedLabel

    styleContext = gtk_widget_get_style_context(app_data.speedLabel);

    gtk_style_context_add_provider(styleContext, GTK_STYLE_PROVIDER(cssProvider), GTK_STYLE_PROVIDER_PRIORITY_APPLICATION);



    // Apply CSS to the backButton (for the camera feed screen)

    styleContext = gtk_widget_get_style_context(app_data.backButton);
